...
std::string BuildEngine::generateVersionJson() const
{
    ...
    if (workspace_) {
        const auto branch = workspace_->findBranch(currentBranch_);
        mcVersion = branch.gameVersion;
        modloader = branch.modloader;
        modloaderVersion = branch.modloaderVersion;
    }

    j["name"] = currentBranch_;
    j["mc_version"] = mcVersion;
    j["modloader"] = modloader;
    j["modloader_version"] = modloaderVersion;
...
std::string BuildEngine::generateHMCLVersionCfg() const
{
    std::string mcVersion;
    if (workspace_) {
        mcVersion = workspace_->findBranch(currentBranch_).gameVersion;
    }

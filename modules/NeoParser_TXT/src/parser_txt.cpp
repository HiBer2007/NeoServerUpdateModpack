...
static bool is_properties_file(const std::string& filepath)
{
...
class TxtConfigParser : public NeoCore::IConfigParser {
public:
    NeoCore::ParserCapability capability() const override
    {
        NeoCore::ParserCapability cap;
        cap.name = "TXT";
        cap.extensions = {".txt", ".properties", ".cfg"};
        cap.supported_modes = {
            NeoCore::TrackingMode::FullSync,
            NeoCore::TrackingMode::LineByLine,
            NeoCore::TrackingMode::NoSync
        };
        cap.supports_line_tracking = true;
        cap.priority = 60;
        return cap;
    }
    ...
    bool can_handle(const std::string& filepath) const override
    {
        ...
        std::string ext = filepath.substr(pos);
        return ext == ".txt" || ext == ".properties" || ext == ".cfg";
    }
        ...
        const std::string& filepath) override
    {
        ...
        if (!is_properties_file(filepath)) {
            return entries;
        }
    ...
    std::vector<NeoCore::LineEntry> parse_lines(
        const std::string& filepath) override
    {
        std::vector<NeoCore::LineEntry> entries;
        ...
        for (size_t i = 0; i < lines.size(); ++i) {
            NeoCore::LineEntry entry;
            entry.line_number = static_cast<int>(i + 1);
            entry.remote_text = lines[i];
            entry.local_text.clear();
            entry.is_tracked = false;
            entries.push_back(entry);
        }
        ...
        const std::string& local_content) override
    {
        ...
        size_t max_lines = std::max(remote_lines.size(), local_lines.size());
        ...
        for (size_t i = 0; i < max_lines; ++i) {
            ...
            bool is_tracked = tracked.count(line_num) > 0;

            std::string output_line;

            if (is_tracked) {
                if (i < remote_lines.size()) {
                    output_line = remote_lines[i];
                }
            } else {
                if (i < local_lines.size()) {
                    output_line = local_lines[i];
                } else if (i < remote_lines.size()) {
                    output_line = remote_lines[i];
                }
            }

            if (!output_line.empty() || i < local_lines.size() || is_tracked) {
                if (!first) {
                    result << "\n";
                }
                result << output_line;
...
{
    return new TxtConfigParser();
}

NEO_DECLARE_PLUGIN_LOG_SINK("NeoParser_TXT")
